...
sanity_check_self.py: Handmade sanity checks for implementation problems
Usage:
...
def question_1h_sanity_check(highway):
    ...
    assert (outp.numpy().shape == outp_expected_size),       "Highway output shape is incorrect it should be:\n{} but is:\n{}".format(outp.numpy().shape, outp_expected_size)
    assert (np.allclose(outp.numpy(), outp_expected.numpy())),         "Highway output is incorrect: it should be:\n {} but is:\n{}".format(outp_expected, outp)
